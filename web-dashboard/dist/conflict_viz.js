import { CARTEL_CLASH_EVENTS } from "./cartel_network_data.generated.js";
...
export class ConflictMap {
    constructor(svgEl) {
        ...
        this.eventsByCartel = new Map();
        this.currentWorld = null;
        this.svg = d3.select(svgEl);
        for (const event of CARTEL_CLASH_EVENTS) {
            if (!this.eventsByCartel.has(event.a))
                this.eventsByCartel.set(event.a, []);
            if (!this.eventsByCartel.has(event.b))
                this.eventsByCartel.set(event.b, []);
    ...
    async init(world) {
        ...
        for (const i of active) {
            ...
            if (!centroids.has(state))
                continue; // defensive: skip if a state name doesn't match the geojson
    ...
    updateNodes(world) {
        ...
        this.nodeGroup
            .selectAll("circle")
            .data(activeIndices, (d) => d)
            .join("circle")
            .attr("cx", (i) => this.positions.get(i).x)
            .attr("cy", (i) => this.positions.get(i).y)
            .attr("r", (i) => 1.5 + 7 * Math.sqrt((world.membersPerCartel[i] ?? 0) / maxMembers))
            .attr("fill", NODE_FILL)
            .attr("fill-opacity", 0.9)
            .style("cursor", "pointer")
            .on("pointerenter", (event, i) => this.showTooltip(event, i))
    ...
    update(world, dtWeeks, title) {
        if (!this.ready)
            return;
        this.currentWorld = world;
        this.updateNodes(world);
        const clashes = world.clashIntensities();
        if (dtWeeks > 0) {
    ...
    renderLabels(world, top, title) {
        ...
        const lines = top.map((c) => `${world.names[c.i] ?? c.i} vs ${world.names[c.j] ?? c.j} · ${c.deathsPerWeek.toFixed(1)}/wk`);
        this.labelText
            .selectAll("tspan")
            .data(lines)
            .join("tspan")
            .attr("x", 14)
            .attr("dy", (_, idx) => (idx === 0 ? 0 : 13))
            .text((d) => d);
    ...
    resetAnimationState() {
        for (const key of this.accumulator.keys())
            this.accumulator.set(key, 0);
        if (this.pulseGroup)
            this.pulseGroup.selectAll("circle").interrupt().remove();
    }
}
//# sourceMappingURL=conflict_viz.js.map