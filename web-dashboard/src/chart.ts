...
export function drawChart(canvas: HTMLCanvasElement, opts: ChartOptions): void {
  ...
  const scaleX = (x: number): number => paddingLeft + ((x - xMin) / (xMax - xMin || 1)) * plotWidth;
  const scaleY = (y: number): number =>
    paddingTop + (1 - (y - yMin) / (yMax - yMin || 1)) * plotHeight;
  ...
  for (const series of opts.series) {
    if (!series.highlightLast || series.data.length === 0) continue;
